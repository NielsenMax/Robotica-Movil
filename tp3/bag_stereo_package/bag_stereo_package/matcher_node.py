...
class MatcherNode(Node):
    ...
    def __process(self, left_msg: Image, right_msg: Image):
        ...
        left_pts = np.array([left_kp[m.queryIdx].pt for m in good_matches], dtype=np.float32).reshape(-1, 1, 2)
        right_pts = np.array([right_kp[m.trainIdx].pt for m in good_matches], dtype=np.float32).reshape(-1, 1, 2)

        # Aplicar RANSAC para calcular la matriz homográfica
        ...
        match_img : cv2.Mat = cv2.drawMatches(left_frame, left_kp, right_frame, right_kp, ransac_matches, None, flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
        ...
        self.matches_pub.publish(self.br.cv2_to_imgmsg(match_img,encoding="bgr8"))
          ...
          # keypoints a numpy
        left_pts_ransac = np.array([left_kp[m.queryIdx].pt for m in ransac_matches], dtype=np.float32).T
        right_pts_ransac = np.array([right_kp[m.trainIdx].pt for m in ransac_matches], dtype=np.float32).T

        # triangular 
        triang_points_hom = cv2.triangulatePoints(self.left_proj, self.right_proj, left_pts_ransac, right_pts_ransac)

        # Convertimos a coordenadas euclidianas
        ...
        #self.get_logger().info('Publishing point cloud')
         ...
         # Transformar los puntos de la imagen izquierda a la derecha usando la matriz homográfica
        left_pts_transformed = cv2.perspectiveTransform(left_pts, H)
        right_frame_bgr = cv2.cvtColor(right_frame, cv2.COLOR_GRAY2BGR)

        # Visualizar los puntos transformados en la imagen derecha
